...
def run_command_callback(ui, edit_widget, text_input):
    ...
    if text_input.endswith("\n"):
        ui.run_command(text_input.strip())
        ui.prompt_bar = None
        ui.set_focus("body")
        ui.reload_footer()
...
class PromptCommand(SameThreadCommand):
    name = "prompt"
    argument_definitions = [Argument("prompt_text", "Text forming the actual prompt", default=":")]
    option_definitions = [Option("input_text", "Text provided on input.")]
...
@register_command
class SearchCommand(SameThreadCommand):
    name = "search"
